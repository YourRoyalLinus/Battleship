    ...
    : postProcessingShader(shader), texture(), width(width), height(height), shake(false)
{
    ...
    this->texture.wrapS = GL_CLAMP_TO_BORDER;
    this->texture.wrapT = GL_CLAMP_TO_BORDER;
    ...
    float offset = 1.0f / 300.0f; // TODO: Large denominator(?) is a hack right now to fix wrapping on screen shake
    float offsets[9][2] = {
